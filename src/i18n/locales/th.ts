import type { MessageSchema } from './en';

const th: MessageSchema = {
    nav: {
        home: 'หน้าแรก',
        aboutMe: 'เกี่ยวกับ',
        techStack: 'เทคโนโลยี',
        projects: 'โปรเจกต์',
        contact: 'ติดต่อ',
    },
    header: {
        resume: 'เรซูเม่',
        language: 'ภาษา',
    },
    hero: {
        status: 'ออนไลน์ · กรุงเทพฯ, ประเทศไทย',
        greeting: 'สวัสดีครับ ผมชื่อ Jhom',
        role: 'ผมเป็น {role}',
    },
    about: {
        whoami: 'สวัสดีครับ!👋{br}ผม {name}',
        from: 'ผมเป็น {role}{br}จากประเทศเมียนมา',
        based: 'ปัจจุบันผมอาศัยอยู่ที่ {city} ประเทศไทย',
        city: 'กรุงเทพฯ',
        specializing: 'เชี่ยวชาญด้าน{br}{field}',
        toBeAdded: 'จะเพิ่มเติมภายหลัง',
        experience: '{years} ปี ในประสบการณ์ด้าน {br} Web Application Development',
        passionTitle: 'สร้างและพัฒนาด้วย Passion',
        passionText: 'ผมหลงใหลในการเขียนโค้ดและการแก้ปัญหา ผมเข้าหาทุกโปรเจกต์ด้วยความคิดสร้างสรรค์ ความทุ่มเท และความใส่ใจโดยการเขียนโค้ดที่สะอาดและดูแลรักษาง่าย',
        swipe: 'ปัด >>>',
    },
    keepCalm: {
        online: 'ออนไลน์',
        story: [
            'ผมเป็นนักพัฒนาที่หลงใหลในการผสมผสานความคิดสร้างสรรค์เข้ากับเทคโนโลยี เพื่อสร้างสรรค์โซลูชันใหม่ ๆ',
            'ไม่ว่าจะเป็นการเขียนโค้ดที่สะอาดและมีประสิทธิภาพ หรือการรับมือกับปัญหาที่ซับซ้อน ผมสนุกกับการเปลี่ยนไอเดียให้กลายเป็นประสบการณ์ดิจิทัลที่สร้างผลลัพธ์ได้จริง',
            'ด้วยพื้นฐานที่แข็งแกร่งด้าน Full-stack development ผมมุ่งมั่นส่งมอบงานที่ทั้งใช้งานได้ดีและน่าสนใจ',
        ],
    },
    techStack: {
        title: 'ประสบการณ์การทำงาน',
        internship: '(ฝึกงาน)',
        present: 'ปัจจุบัน',
        internshipDetail: 'ผมใช้ Wordpress, HTML, CSS, Javascript, MySQL และ Google Cloud',
        omnistarDetail: 'ผมดูแลงานด้านโครงสร้างพื้นฐาน ตั้งค่าฐานข้อมูล SQL และจัดการการ deploy',
        clicknextDetail: 'Tech stack ปัจจุบันของผมประกอบด้วย Vue, Typescript, SCSS, C# และ Microsoft SQL Server รวมถึงใช้ Git และ Vim เป็นเครื่องมือในการพัฒนา',
        currentlyUsing: 'เทคโนโลยีที่ผมใช้อยู่ในปัจจุบัน',
        usedAndFamiliar: 'เทคโนโลยีที่ผมเคยใช้และคุ้นเคย',
        familiar: 'เทคโนโลยีที่ผมคุ้นเคย',
    },
    projects: {
        title: 'โปรเจกต์ส่วนตัว',
        subtitle: 'โปรเจกต์ส่วนตัวของผมบางส่วนที่ผมชื่นชอบ',
        tools: 'เครื่องมือ',
        guitar: 'กีตาร์',
        fretwizard: 'FretWizard คือเฟรตบอร์ดแบบอินเทอร์แอกทีฟ สำหรับดูสเกล คอร์ด และแพทเทิร์นต่าง ๆ ทั่วทั้งคอกีตาร์',
        github: 'แวะไปดูโปรเจกต์อื่น ๆ ได้ที่ GitHub ของผมครับ',
    },
    contact: {
        title: 'ติดต่อผม',
        intro: 'สามารถติดต่อผมได้ทางอีเมล หรือส่งข้อความผ่านแบบฟอร์มนี้ได้เลย',
        copy: 'คัดลอก',
        copied: 'คัดลอกแล้ว!',
        form: {
            title: 'ส่งข้อความถึงผม',
            firstName: 'ชื่อ',
            lastName: 'นามสกุล',
            email: 'อีเมลของคุณ',
            message: 'ข้อความ',
            send: 'ส่ง',
            sending: 'กำลังส่ง...',
            sent: 'ส่งแล้ว!',
            tryAgain: 'ลองอีกครั้ง',
            thanks: 'ขอบคุณครับ! ผมจะติดต่อกลับโดยเร็วที่สุด',
            error: 'เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง หรือคัดลอกอีเมลของผม',
        },
    },
};

export default th;
